    ...
    for (_, mount) in mounts.iter() {
            ...
            && (mount.source_type == 0 || [1257].contains(&mount.id))
        {
            ...
            if debug {
                println!(
                    "list count: {}",
                    listview_reg.captures_iter(item_html.as_str()).count()
                );
            }
            if listviews2.count() == 1 {
                for drop_data in listviews {
                    let drop_data: Value =
                        serde_json::from_str(drop_data.get(1).unwrap().as_str()).unwrap();
                    let npc = drop_data
                        .as_array()
                        .unwrap()
                        .first()
                    ...
                    if classification == 2
                        || classification == 4
                        || NPC_WHITELIST.contains(&npc_id)
                        || (classification == 0 && false == npc.contains_key("boss"))
                    {
                        // is rare elite || rare || world boss? || zone mob
                        ...
                        for position_data in mapper_reg.captures_iter(npc_html.as_str()) {
                            ...
                            if map_id.is_some() {
                                ...
                                let coords = position_data
                                    ...
                                    .as_array()
                                    .unwrap();
                                let map_x: u16 = (coords.get(0).unwrap().as_f64().unwrap() * 100.0)
                                    .round()
                                    as u16;
                                let map_y: u16 = (coords.get(1).unwrap().as_f64().unwrap() * 100.0)
                                    .round()
                                    as u16;
